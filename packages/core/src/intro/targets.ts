 ...
 * 记在这里而不是绕过去。
 *
 * ### 已知问题（已拍板）：中英两档「同一档」不成立，缺口按内容问题处置
 *
 ...
 * 英文那一侧是「按这个换算根本填不满」。
 *
 * ### 拍板（2026-09-19，fiber）：常数不动，缺口按内容问题处置
 *
 * 先把两个量分清楚 —— 它们不是竞争关系，量的是**不同的东西**：
 *
 * - **0.6 是语速比的产物**（240 字/分 ÷ 150 词/分 ≈ 0.625），量的是**时间**：
 *   念 60 秒各是多少字 / 词。它不包含翻译等价性 —— 原假设之所以曾经显得
 *   成立，是因为忠实翻译的中英密度恰好也是 ~0.6 词/字，两个独立的量碰巧接近。
 * - **0.38 是本项目渲染产物的内容密度**，量的是**内容**：它被「英版允许删
 *   非数字事实」（DESIGN 10.2 的不对称）污染，且 n=1（单一轻样本）。
 *   **永远不要拿它当换算常数用** —— `intro.test.ts` 钉它只为让渲染密度的
 *   漂移可见。
 *
 * 决定性的算术：即使按忠实的 0.6 换算，209 字也只折 125 词，仍低于
 * `intro_en@60s` 档下限 135 —— **无论选哪个常数，轻样本都过不了英文 60 秒档**。
 * 缺口的主导项是内容（一页校招档案可朗读的事实撑不满英文一分钟），不是常数。
 * 调常数最多把忠实翻译的及格线救回来（英文语速降到 130 wpm 时 125 词落档内），
 * 救不了实际渲染的量（79 词仍只有 67%）。
 *
 * 所以：
 *
 * 1. **语速常数 {4.0, 2.5} 不动**（都在真实区间）。为过测试调常数是把温度计
 *    拆掉 ——「中英同一档」会从「被证伪」变成「被凑出来」。
 * 2. **`check.ts` 照 ±10% 判，轻样本上 `en@60s` 诚实报缺**（DESIGN 11.5
 *    硬约束；失败详情里保留「这可能不是档案的问题」）。对一份真实校招档案，
 *    英文一分钟说不满本来就是要暴露给用户的信号，不是要用常数藏掉的缺陷。
 * 3. 若未来要填这个缺口，动的是**英文版的删事实尺度**（DESIGN 10.2 的不对称），
 *    不是这里的常数。
 */
